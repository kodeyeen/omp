...
func NewPlayerObject(plr *Player, modelID int, drawDist float32, pos Vector3, rot Vector3) (*PlayerObject, error) {
	cObj := C.playerObject_create(plr.handle, C.int(modelID), C.float(pos.X), C.float(pos.Y), C.float(pos.Z), C.float(rot.X), C.float(rot.Y), C.float(rot.Z), C.float(drawDist))
	if cObj == nil {
		return nil, errors.New("object limit was reached")
	}
...
func (o *PlayerObject) DrawDistance() float32 {
	return float32(C.playerObject_getDrawDistance(o.handle))
}

func (o *PlayerObject) SetModel(model int) {
	C.playerObject_setModel(o.handle, C.int(model))
}

func (o *PlayerObject) Model() int {
	return int(C.playerObject_getModel(o.handle))
}

func (o *PlayerObject) EnableCameraCollision() {
	C.playerObject_setCameraCollision(o.handle, C.uchar(1))
}

func (o *PlayerObject) DisableCameraCollision() {
	C.playerObject_setCameraCollision(o.handle, C.uchar(0))
}

func (o *PlayerObject) IsCameraCollisionEnabled() bool {
	return C.playerObject_getCameraCollision(o.handle) != 0
}

func (o *PlayerObject) Move(pos Vector3, rot Vector3, speed float32) time.Duration {
	millis := C.playerObject_move(o.handle, C.float(pos.X), C.float(pos.Y), C.float(pos.Z), C.float(rot.X), C.float(rot.Y), C.float(rot.Z), C.float(speed))

	return time.Duration(millis) * time.Millisecond
}

func (o *PlayerObject) IsMoving() bool {
	return C.playerObject_isMoving(o.handle) != 0
}

func (o *PlayerObject) Stop() {
	C.playerObject_stop(o.handle)
}

func (o *PlayerObject) MovingData() ObjectMoveData {
	data := C.playerObject_getMovingData(o.handle)
...
func (o *PlayerObject) AttachToVehicle(veh *Vehicle, offset Vector3, rot Vector3) {
	C.playerObject_attachToVehicle(o.handle, veh.handle, C.float(offset.X), C.float(offset.Y), C.float(offset.Z), C.float(rot.X), C.float(rot.Y), C.float(rot.Z))
}

func (o *PlayerObject) ResetAttachment() {
	C.playerObject_resetAttachment(o.handle)
}

// func PlayerObjectAttachedData[T ObjectAttachmentTarget](obj *Object) (ObjectAttachmentData[T], error) {
// 	data := C.playerObject_getAttachmentData(obj.handle)
...
// 		target = Vehicle{handle: veh}
// 	case *Object:
// 		obj := C.playerObject_getByID(data.ID)
// 		if obj == nil {
// 			return result, errors.New("object is not attached to an object")
// 		}
...
func (o *PlayerObject) IsMaterialSlotUsed(slotIdx int) bool {
	return C.playerObject_isMaterialSlotUsed(o.handle, C.uint(slotIdx)) != 0
}

func (o *PlayerObject) Material(slotIdx int) (ObjectMaterial, error) {
	var data C.ObjectMaterial
	res := C.playerObject_getMaterial(o.handle, C.uint(slotIdx), &data) != 0
	if !res {
		return ObjectMaterial{}, errors.New("invalid slot index is specified")
	}
...
func (o *PlayerObject) MaterialText(slotIdx int) (ObjectMaterialText, error) {
	var data C.ObjectMaterialText
	res := C.playerObject_getMaterialText(o.handle, C.uint(slotIdx), &data) != 0
	if !res {
		return ObjectMaterialText{}, errors.New("invalid slot index is specified")
	}
...
func (o *PlayerObject) SetMaterial(idx, model int, textureLib, textureName string, color int) {
	...
	C.playerObject_setMaterial(o.handle, C.uint(idx), C.int(model), cTextureLib, cTextureName, C.uint(color))
}
...
func (o *PlayerObject) SetMaterialText(
	...
	C.playerObject_setMaterialText(o.handle, C.uint(slotIdx), cText, C.ObjectMaterialSize(size), cFontFace, C.int(fontSize), newCUchar(bold), C.uint(fontColor), C.uint(bgColor), C.ObjectMaterialTextAlign(align))
}

func (o *PlayerObject) AttachToPlayer(plr *Player, offset Vector3, rot Vector3) {
	C.playerObject_attachToPlayer(o.handle, plr.handle, C.float(offset.X), C.float(offset.Y), C.float(offset.Z), C.float(rot.X), C.float(rot.Y), C.float(rot.Z))
}

func (o *PlayerObject) SetPosition(pos Vector3) {
	C.playerObject_setPosition(o.handle, C.float(pos.X), C.float(pos.Y), C.float(pos.Z))
}

func (o *PlayerObject) Position() Vector3 {
	pos := C.playerObject_getPosition(o.handle)
...
func (o *PlayerObject) SetRotation(rot Vector3) {
	C.playerObject_setRotation(o.handle, C.float(rot.X), C.float(rot.Y), C.float(rot.Z))
}

func (o *PlayerObject) Rotation() Vector3 {
	rot := C.playerObject_getRotation(o.handle)
